"""
오늘자 숙소 온라인 가격 크롤링 배치 작업
- today_accommodation_info 테이블의 모든 레코드에 대해 네이버 호텔에서 가격 크롤링
...
async def get_today_accommodation_records() -> List[Dict]:
    """
    today_accommodation_info 테이블의 모든 레코드 가져오기
    숙소 정보와 JOIN하여 이름과 방 타입도 함께 가져옴
    ...
    async with AsyncSessionLocal() as db:
        try:
            logger.info("Fetching today_accommodation_info records...")

            # today_accommodation_info와 accommodations를 JOIN하여 조회
            result = await db.execute(
                select(
                    TodayAccommodation.id,
                    TodayAccommodation.accommodation_id,
            ...
            logger.info(f"Found {len(records)} today_accommodation_info records to process")
            return records

        except Exception as e:
            logger.error(f"Error fetching today_accommodation_info records: {str(e)}")
            return []
...
async def update_online_price_in_db(today_id: str, online_price: float) -> bool:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            await db.rollback()
            logger.error(f"Error updating online_price for {today_id}: {str(e)}")
            raise


async def process_today_accommodation_price_crawler() -> Dict:
    """
    오늘자 숙소 온라인 가격 크롤링 메인 함수
    """
    async with async_playwright() as p:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Starting today accommodation price crawler batch job...")
            logger.info("Using Naver Hotel (hotel.naver.com)")
            logger.info("=" * 60)

            # 처리할 레코드 가져오기
            records = await get_today_accommodation_records()

            if not records:
                logger.warning("No today_accommodation_info records to process")
                return {
                    "status": "warning",
            ...
            for idx, record in enumerate(records, 1):
                ...
                try:
                    ...
                    logger.warning(f"Error processing record {record['today_id']}: {str(e)}")
                    total_failed += 1
                    continue
...
def handler(event, context):
    """AWS Lambda 핸들러"""
    try:
        result = asyncio.run(process_today_accommodation_price_crawler())
        return {
            "statusCode": 200,
            "body": result
        }
