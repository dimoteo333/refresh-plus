...
async def process_wishlist_notification_evening():
    """
    찜한 날짜 신청 오픈 알림 (저녁)
    - wishlists와 today_accommodation_info 매칭
    - 신청 가능 상태인 경우 알림 발송
    - 8시간 중복 방지 (오전 알림과 중복 방지)
    """
    async with AsyncSessionLocal() as db:
        try:
            logger.info("=" * 80)
            logger.info("Starting wishlist notification (evening)")
            logger.info("=" * 80)
            ...
            logger.info("=" * 80)
            logger.info(f"Wishlist notification (evening) completed")
            ...
            logger.error(f"Wishlist notification (evening) failed: {str(e)}", exc_info=True)
            return {
                "status": "error",
                "message": str(e),
                "timestamp": datetime.utcnow().isoformat()
